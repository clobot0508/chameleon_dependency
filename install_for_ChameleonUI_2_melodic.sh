...
echo ""
echo "Install_for_ChameleonUI_2 for ROS MELODIC"
echo ""
echo ""
echo ""
...
if [ -n "$keyword_alias" ]; then	
	...
	echo "install_for_ChameleonUI_1_melodic.sh를 실행시켜 nvm 설치 해주세요!!!"
...
if [ -n "$keyword_alias" ]; then	
	...
	echo "install_for_ChameleonUI_v_melodic.sh를 실행시켜 redis-cli 설치 해주세요!!!"
	echo "[The Second Installation Incompleted!!!]"
	exit 0
fi
...
echo ""
echo ""
echo "[Check whether pm2 is installed]"
...
sudo python2.7 -m pip install redis

sudo apt-get install ros-melodic-pointcloud-to-laserscan ros-melodic-rospy-message-converter -y
